...
public class GamePlayer : MonoBehaviour
{
    ...
    public void DigUp(object modifier)
    {
        ...
        GM.DigUpResult.GetComponent<FouilleResultManager>().UpdateResult(rsr.Name);
        GM.DigUpResult.SetActive(true);
        StartCoroutine(DesableDigUpResult());
        
        p.Inventory.Add(rsr);

        int[] resources = GM.EM.Scenario.CountPlayersResources();
        GM.ObjectiveDisplayManager.GetComponent<ObjectifsCountManager>().UpdateObjectifCounts(resources[0], resources[1], resources[2], resources[3]);
    ...
    IEnumerator DesableDigUpResult()
    {
        yield return new WaitForSeconds(2);
        GM.DigUpResult.SetActive(false);
    }
    ...
    public void Fight()
    {
        ...
        if (TargetEnemy.Threat <= Craftiness + dice)
        { // + BONUS Craftiness
            int dmg = Craftiness + dice - TargetEnemy.Threat;
            Debug.Log(Name + "hurts " + TargetEnemy.Name + " [" + dmg + "damage(s)]");
            bool isDead = TargetEnemy.Hurt(dmg); // TODO + BONUS Damage

            GM.FightScreen.GetComponent<FightDisplayMan>().GenerateCombatText(Craftiness, TargetEnemy.Threat, dice, dmg, "ATK", true, Name, "The Cat", isDead);
            GM.FightScreen.GetComponent<FightDisplayMan>().UpdateStatus(Craftiness, Liveliness, TargetEnemy.Threat, TargetEnemy.Life, Name, "cat", true);
        ...
        else
        {
            ...
            GM.FightScreen.GetComponent<FightDisplayMan>().GenerateCombatText(Craftiness, TargetEnemy.Threat, dice, 0, "ATK", false, Name, "The Cat");
            GM.FightScreen.GetComponent<FightDisplayMan>().UpdateStatus(Craftiness, Liveliness, TargetEnemy.Threat, TargetEnemy.Life, Name, "cat", true);
